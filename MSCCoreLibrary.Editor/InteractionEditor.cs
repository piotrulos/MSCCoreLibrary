        ...
        public override void OnInspectorGUI()
        {
            ...
            for (int i = 0; i < interactions.Count; i++)
            {
                ...
                if (showElement[i])
                {
                    ...
                    {
                        iconTexture = AssetDatabase.LoadAssetAtPath(string.Format("Assets/Scripts/Editor/{0}.png", icons[selectedIcon]), typeof(Texture2D)) as Texture2D;
                        if (iconTexture == null)
                        {
                            EditorGUILayout.HelpBox(string.Format("Icon not found in Assets/Scripts/Editor/{0}.png!", icons[selectedIcon]), MessageType.Warning);
                        }
                    }
        ...
        private void DrawInteractionEventUI(InteractionConfig interactionConfig, int eventIndex, InteractionEvent interactionEvent, int interactionIndex)
        {
            ...
            if (showInteractionEventDetails[interactionIndex].showInteraction[eventIndex])
            {
                ...
                if (eventIndex == 3) // Scroll wheel
                {
                    ...
                    SerializedProperty onMouseScrollUpProp = serializedObject.FindProperty("interactions").GetArrayElementAtIndex(interactionIndex).FindPropertyRelative("interactionEvents").GetArrayElementAtIndex(eventIndex).FindPropertyRelative("OnScrollUp");
                    EditorGUILayout.PropertyField(onMouseScrollUpProp);
                    SerializedProperty onMouseScrollDownProp = serializedObject.FindProperty("interactions").GetArrayElementAtIndex(interactionIndex).FindPropertyRelative("interactionEvents").GetArrayElementAtIndex(eventIndex).FindPropertyRelative("OnScrollDown");
                    EditorGUILayout.PropertyField(onMouseScrollDownProp);
                }
                else
                {
                    if (interactionEvent.onHold)
